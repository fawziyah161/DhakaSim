...
public class Segment {
    ...
    private int contactCount  = 0; // optional
...
//        g2d.drawLine((x1 + x3) / 2, (y1 + y3) / 2, (x2 + x4) / 2, (y2 + y4) / 2); // divider
    ...
    public void updateContactCount()  { contactCount++;  }

    int getAccidentCount() {
        return accidentCount;
    }
    int getContactCount()  { return contactCount;  }
    static class SafetyEvent {
        public final long step;
        public final int v1, v2;
        public final String type; // "contact", "crash_minor", "crash_major", "near_miss"
        public final double ttc, dV;

        SafetyEvent(long step, int v1, int v2, String type, double ttc, double dV) {
            this.step = step; this.v1 = v1; this.v2 = v2; this.type = type; this.ttc = ttc; this.dV = dV;
        }
    }

    private final java.util.List<SafetyEvent> safetyLog = new java.util.ArrayList<>();
    public void logEvent(long step, int v1, int v2, String type, double ttc, double dV) {
        safetyLog.add(new SafetyEvent(step, v1, v2, type, ttc, dV));
        if (type.startsWith("crash")) updateAccidentcount();
        if ("contact".equals(type))   updateContactCount();
    }
    public java.util.List<SafetyEvent> getSafetyLog() { return safetyLog; }
